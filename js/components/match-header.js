...
injectStyle('match-header', `
  ...
  .match-header__row { display: grid; grid-template-columns: minmax(0, 1fr) auto minmax(0, 1fr); align-items: center; gap: var(--sp-sm); }
  .match-header__team { display: flex; align-items: center; justify-content: center; text-align: center; }
  .match-header__team-name { font-family: var(--font-display); font-size: var(--fs-lg); font-weight: 700; overflow-wrap: anywhere; }
  ...
  return `
    <div class="match-header">
      ...
      <div class="match-header__row">
        <div class="match-header__team"><span class="match-header__team-name">${match.homeTeam.name}</span></div>
        <div class="match-header__center">${centerContent}${statusLine}</div>
        <div class="match-header__team"><span class="match-header__team-name">${match.awayTeam.name}</span></div>
      </div>
