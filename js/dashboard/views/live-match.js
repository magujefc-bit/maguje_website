...
injectStyle('live-match-view', `
  ...
  .squad-check-row input {
    width: auto;
  }
`);

export async function liveMatchView(params, query) {
  const admin = await requireAdmin(['match_manager', 'content_manager']);
  ...
  viewContainer.render(`
    ...
    <div class="card" id="pre-start-card">
      ...
      <div
        id="starter-select-list"
        style="max-height:280px; overflow-y:auto; border:1px solid #f0f4f1; border-radius:8px; padding:0.4rem;"
      ></div>
    ...
    <div class="card hidden" id="live-controls-card">
      ...
      <div class="action-grid">
        <div class="action-btn" id="record-our-goal-btn">
          ⚽ Our Goal
        </div>

        <div class="action-btn" id="record-opponent-goal-btn">
          🥅 Opponent Goal
        </div>

        <div class="action-btn" id="record-card-btn">
          🟨 Record Card
        </div>

        <div class="action-btn" id="record-sub-btn">
          🔄 Substitution
        </div>
      </div>
      ...
      <div class="modal-box">
        ...
        <div id="goal-our-fields">
          <div class="field-grid">
            <div>
              <label>Scorer</label>
              <select id="goal-scorer-select"></select>
            </div>

            <div>
              <label>Assist (optional)</label>
              <select id="goal-assist-select">
                <option value="">—</option>
              </select>
            </div>
          </div>
        ...
        <div id="goal-opponent-note" class="hidden">
          <p class="empty-msg">
            This records a goal for the opponent — no scorer needed, just the time.
          </p>
        </div>

        <div class="modal-actions">
          <button class="btn-secondary" id="goal-cancel-btn">
            Cancel
          </button>

          <button class="btn-primary" id="goal-save-btn">
            Save Goal
          </button>
        </div>
    ...
    <div class="modal-overlay hidden" id="card-modal">
      <div class="modal-box">
        ...
        <div class="field-grid">
          <div>
            <label>Player (on pitch)</label>
            <select id="card-player-select"></select>
          </div>
        ...
        <div class="modal-actions">
          <button class="btn-secondary" id="card-cancel-btn">
            Cancel
          </button>

          <button class="btn-primary" id="card-save-btn">
            Save Card
          </button>
        </div>
    ...
    <div class="modal-overlay hidden" id="sub-modal">
      <div class="modal-box">
        ...
        <div class="field-grid">
          <div>
            <label>Player Out (on pitch)</label>
        ...
        <div class="modal-actions">
          <button class="btn-secondary" id="sub-cancel-btn">
            Cancel
          </button>

          <button class="btn-primary" id="sub-save-btn">
            Save Substitution
          </button>
        </div>
    ...
    <div class="modal-overlay hidden" id="draw-modal">
      <div class="modal-box">
        ...
        <p class="empty-msg">
          The match is tied. What happens next?
        </p>

        <div id="draw-continue-option">
          <div class="field-grid">
            <div>
              <label>Extra Time Length (minutes)</label>
              <input
                type="number"
                id="et-length-input"
                value="15"
          ...
          <button
            class="btn-primary"
            id="draw-continue-btn"
            style="width:100%; margin-bottom:0.6rem;"
          >
            ▶️ Continue — Extra Time
          </button>
        </div>

        <button
          class="btn-secondary"
          id="draw-penalties-btn"
          style="width:100%; margin-bottom:0.6rem;"
        >
          🥅 Go to Penalties
        </button>
        ...
        <div class="modal-actions">
          <button class="btn-secondary" id="draw-cancel-btn">
            Cancel
          </button>
        </div>
    ...
    <div class="modal-overlay hidden" id="penalty-modal">
      <div class="modal-box">
        ...
        <p class="empty-msg">
          The score stays as the drawn result — this just records who progressed.
        </p>
        ...
        <div class="modal-actions">
          <button class="btn-secondary" id="pen-cancel-btn">
            Cancel
          </button>
        </div>
      </div>
  ...
  function teamName(id) {
    return (allTeams.find(t => t.id === id) || {}).name || 'Unknown team';
  }
  ...
  async function loadMatch() {
    ...
    document.getElementById('back-link').href =
      `${dashPath('/competitions/detail')}?id=${data.competition_id}`;

    if (!currentMatch.is_internal) {
      document.getElementById('main').innerHTML = `
        <h1>Live tracking is only available for our own matches</h1>
        <p class="sub">
          <a href="${dashPath('/competitions/detail')}?id=${data.competition_id}">
            Go back
          </a>
        </p>
      `;
    ...
    if (currentMatch.status === 'completed') {
      document.getElementById('main').innerHTML = `
        <h1>This match is already completed and locked</h1>
        <p class="sub">
          <a href="${dashPath('/competitions/detail')}?id=${data.competition_id}">
            Go back
          </a>
    ...
    const homeSide = currentMatch.is_home
      ? ourClubName
      : opponentName;

    const awaySide = currentMatch.is_home
      ? opponentName
      : ourClubName;
    ...
    document.getElementById('page-sub').textContent =
      `${currentMatch.match_date || ''} ${currentMatch.match_time || ''} · ${currentMatch.venue || ''}`;
  ...
  function computeSquadSets() {
    ...
    const subbedOffIds = new Set(
      subs.map(s => s.player_out_id)
    );

    const subbedOnIds = new Set(
      subs.map(s => s.player_in_id)
    );

    const onPitch = new Set(
      [...starterIds].filter(id => !subbedOffIds.has(id))
    );

    subbedOnIds.forEach(id => onPitch.add(id));

    const recordedIds = new Set(
      matchLineups.map(l => l.player_id)
    );
  ...
  function renderStarterChecklist() {
    ...
    container.innerHTML = allPlayers
      .map(p => `
        <label class="squad-check-row">
          <input
            type="checkbox"
            class="starter-checkbox"
            value="${p.id}"
          >
          <span style="flex:1;">
            ${escapeHtml(p.team_name || p.full_name)}
          </span>
        </label>
      `)
      .join('');
    ...
    container
      .querySelectorAll('.starter-checkbox')
      .forEach(cb => {
  ...
  function updateStarterCount() {
    const count =
      document.querySelectorAll('.starter-checkbox:checked').length;
  ...
  document
    ...
    .addEventListener('click', async () => {
      ...
      if (starterIds.length < 1) {
        statusEl.textContent =
          'Select at least 1 starting player.';
        statusEl.classList.add('error');
        return;
      }
      ...
      if (starterIds.length > 11) {
        statusEl.textContent =
          "Starting XI can't exceed 11 players.";
        statusEl.classList.add('error');
        return;
      }
      ...
      try {
        const lineupRows = starterIds.map(id => ({
          match_id: matchId,
          player_id: id,
          is_starter: true
        }));

        const { error: lineupError } = await supabaseClient
          .from('match_lineups')
          .insert(lineupRows);
  ...
  function renderControls() {
    ...
    if (currentMatch.live_state === 'first_half') {
      phaseRow.innerHTML = `
        <button class="btn-secondary" id="half-time-btn">
          ⏸️ Half Time
        </button>
      `;

      document
        .getElementById('half-time-btn')
        .addEventListener('click', async () => {
          await updateMatch({
            live_state: 'half_time'
          });
        });
    } else if (currentMatch.live_state === 'half_time') {
      phaseRow.innerHTML = `
        <button class="btn-primary" id="start-2nd-btn">
          ▶️ Start 2nd Half
        </button>
      `;

      document
        .getElementById('start-2nd-btn')
        .addEventListener('click', async () => {
          await updateMatch({
            live_state: 'second_half',
            second_half_started_at: new Date().toISOString()
          });
        });
    } else if (currentMatch.live_state === 'second_half') {
      phaseRow.innerHTML = `
        <button class="btn-danger" id="end-game-btn">
          ⏹️ End Game
        </button>
      `;
      ...
      document
        ...
        .addEventListener('click', () => {
          handleEndGameClick(false);
        });
    } else if (currentMatch.live_state === 'extra_time') {
      phaseRow.innerHTML = `
        <button class="btn-danger" id="end-game-btn">
          ⏹️ End Game (Extra Time)
        </button>
      `;
  ...
  document
    .getElementById('record-our-goal-btn')
    .addEventListener('click', () => {
      pendingGoalSide = 'us';
      ...
      document
        .getElementById('goal-our-fields')
        .classList.remove('hidden');

      document
        .getElementById('goal-opponent-note')
        .classList.add('hidden');

      document.getElementById('goal-scorer-select').innerHTML =
        playerOptionsFromSet(squadSets.onPitch, false);

      document.getElementById('goal-assist-select').innerHTML =
        playerOptionsFromSet(squadSets.onPitch, true);
  ...
  document
    .getElementById('record-opponent-goal-btn')
    .addEventListener('click', () => {
  ...
  document
    .getElementById('goal-cancel-btn')
    .addEventListener('click', () => {
      document
        .getElementById('goal-modal')
        .classList.add('hidden');
  ...
  document
    ...
    .addEventListener('click', async () => {
      ...
      const payload = pendingGoalSide === 'opponent'
        ? {
            match_id: matchId,
            is_opponent_goal: true,
            scorer_id: null,
            minute
          }
        : {
            match_id: matchId,
            is_opponent_goal: false,
            scorer_id:
              document.getElementById('goal-scorer-select').value,
            assist_id:
              document.getElementById('goal-assist-select').value || null,
            minute
          };
  ...
  document
    .getElementById('record-card-btn')
    .addEventListener('click', () => {
      document.getElementById('card-player-select').innerHTML =
        playerOptionsFromSet(squadSets.onPitch, false);
  ...
  document
    ...
    .addEventListener('click', async () => {
      ...
      const payload = {
        match_id: matchId,
        player_id:
          document.getElementById('card-player-select').value,
        card_type:
          document.getElementById('card-type-select').value,
        minute: ScoreboardCore.currentMinute(currentMatch)
      };
  ...
  document
    .getElementById('record-sub-btn')
    .addEventListener('click', () => {
      document.getElementById('sub-out-select').innerHTML =
        playerOptionsFromSet(squadSets.onPitch, false);

      document.getElementById('sub-in-select').innerHTML =
        playerOptionsFromSet(squadSets.bench, false);

      document.getElementById('sub-status').textContent = '';

      document
        .getElementById('sub-modal')
        .classList.remove('hidden');
    });

  document
    .getElementById('sub-cancel-btn')
    .addEventListener('click', () => {
      document
        .getElementById('sub-modal')
        .classList.add('hidden');
  ...
  document
    .getElementById('sub-save-btn')
    .addEventListener('click', async () => {
      const statusEl = document.getElementById('sub-status');

      const playerOut =
        document.getElementById('sub-out-select').value;

      const playerIn =
        document.getElementById('sub-in-select').value;

      if (!playerOut || !playerIn) {
        statusEl.textContent = 'Both players are required.';
        statusEl.classList.add('error');
        return;
      }
      ...
      if (playerOut === playerIn) {
        statusEl.textContent =
          "Player out and in can't be the same.";
        statusEl.classList.add('error');
        return;
      }
      ...
      const { error: lineupErr } = await supabaseClient
        .from('match_lineups')
        .insert({
          match_id: matchId,
          player_id: playerIn,
      ...
      if (lineupErr && lineupErr.code !== '23505') {
        statusEl.textContent =
          `Substitution saved, but lineup update failed: ${lineupErr.message}`;
  ...
  function handleEndGameClick(isExtraTime) {
    const ourScore =
      goals.filter(g => !g.is_opponent_goal).length;
  ...
  function openDrawModal(isExtraTime) {
    document
      .getElementById('draw-continue-option')
      .classList.toggle('hidden', isExtraTime);

    document.getElementById('draw-status').textContent = '';

    document
      .getElementById('draw-modal')
      .classList.remove('hidden');
  }

  document
    .getElementById('draw-cancel-btn')
    .addEventListener('click', () => {
      document
        .getElementById('draw-modal')
        .classList.add('hidden');
  ...
  document
    .getElementById('draw-continue-btn')
    .addEventListener('click', async () => {
      const etLen =
        parseInt(
          document.getElementById('et-length-input').value
        ) || 15;
      ...
      await updateMatch({
        live_state: 'extra_time',
        extra_time_started_at: new Date().toISOString(),
        extra_time_length_minutes: etLen
      });
    });

  document
    .getElementById('draw-penalties-btn')
    .addEventListener('click', () => {
      document
        .getElementById('draw-modal')
        .classList.add('hidden');
      ...
      document
        .getElementById('penalty-modal')
        .classList.remove('hidden');
    });

  document
    .getElementById('draw-record-btn')
    .addEventListener('click', async () => {
      document
        .getElementById('draw-modal')
        .classList.add('hidden');
  ...
  document
    .getElementById('pen-opponent-btn')
    .addEventListener('click', async () => {
      document
        .getElementById('penalty-modal')
        .classList.add('hidden');
  ...
  async function finalizeMatch(extra) {
    ...
    router.navigate(
      `${dashPath('/competitions/detail')}?id=${currentMatch.competition_id}`,
      { replace: true }
    );
  }
