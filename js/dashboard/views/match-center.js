...
injectStyle('match-center-view', `
  ...
  .filter-popover .radio-line input {
    width: auto;
  }
...
export async function matchCenterView() {
  ...
  viewContainer.render(`
    ${pageHeader(
      'Match Center',
      'Browse teams and matches across all competitions.'
    )}
    ...
    <div class="tabs">
      ...
      <button
        class="tab-btn ${admin.role === 'content_manager' ? 'active' : ''}"
        data-tab="matches"
      >
        Matches
      </button>
            ...
            >
              ...
              <div class="radio-line">
                <input
                  ...
                  name="sort-mode"
                  value="appearances"
                  id="sort-mode-appearances"
                >
                <label
                  for="sort-mode-appearances"
                  style="margin:0;"
                >
                  Appearances
                </label>
              ...
              <div class="radio-line">
                <input
                  ...
                  name="sort-mode"
                  value="assists"
                  id="sort-mode-assists"
                >
                <label
                  for="sort-mode-assists"
                  style="margin:0;"
                >
                  Assists
                </label>
    ...
    <div
      class="tab-panel ${admin.role === 'content_manager' ? '' : 'active'}"
      id="tab-teams"
    >
  ...
  async function loadClubName() {
    ...
    document.getElementById('our-club-row').textContent =
      `⭐ ${ourClubName} (this is your club)`;
  }
  ...
  async function loadPlayerStats() {
    const statusEl =
      document.getElementById('player-stats-status');
  ...
  document
    .getElementById('player-stats-search')
    .addEventListener(
      'input',
      renderPlayerStatsTable
    );
  ...
  function renderPlayerStatsTable() {
    ...
    const tbody =
      document.getElementById('player-stats-tbody');
  ...
  document.addEventListener('click', e => {
    ...
    const popover =
      document.getElementById('sort-popover');

    if (
      popover &&
      !popover.contains(e.target) &&
      e.target.id !== 'sort-toggle-btn'
    ) {
  ...
  document
    .getElementById('sort-apply-btn')
    .addEventListener('click', () => {

      sortMode =
        document
          .querySelector(
            'input[name="sort-mode"]:checked'
          )
          .value;
      ...
      document
        .getElementById('active-sort-label')
        .textContent = labels[sortMode];
    ...
    filtered.forEach(t => {
      ...
      row.innerHTML = `
        ...
        <div class="item-actions">
          ...
          <button class="btn-secondary rename-btn">
            Rename
          </button>
      ...
      row
        ...
        .addEventListener('click', async () => {
          ...
          if (
            !confirm(
              `Delete team "${t.name}"? ` +
              `This will fail if it's used in any match.`
            )
          ) {
            return;
          }
  ...
  document
    ...
    .addEventListener('click', () => {
      ...
      if (filterCompetitionId) {
        labelParts.push(
          competitionName(filterCompetitionId)
        );
      }

      if (filterStatus) {
        labelParts.push(filterStatus);
      }

      document
        .getElementById('active-filter-label')
        .textContent =
          labelParts.length
            ? labelParts.join(' · ')
            : 'Sorted by date';
  ...
  function renderMatchCard(m) {
    ...
    if (m.is_internal) {
      ...
      matchupHtml =
        `${escapeHtml(homeSide)} vs ` +
        `${escapeHtml(awaySide)}`;

    } else {

      matchupHtml =
        `${escapeHtml(teamName(m.team_a_id))} vs ` +
        `${escapeHtml(teamName(m.team_b_id))}`;
    }
      ...
      <div class="match-top">
        ...
        <div>
          ...
          <div class="match-meta">
            ${escapeHtml(
              competitionName(m.competition_id)
            )}
            ·
