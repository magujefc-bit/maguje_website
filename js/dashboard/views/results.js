...
injectStyle('results-view', `
  ...
  .standings-table tr.own-club-row {
    background: #eaf6ee;
    font-weight: 700;
  }
  ...
  .squad-table input[type="text"] {
    width: 100px;
  }

  .squad-table input[type="radio"] {
    margin-right: 4px;
  }
...
export async function resultsView(params, query) {
  ...
  viewContainer.render(`
    ...
    <div id="list-view">
      ...
      <p class="sub">
        Standings and results by competition.
      </p>
    ...
    >
      ...
      >
        ...
        <p class="empty-msg">
          Select who's in the squad, and whether
          they started or were on the bench.
        </p>
      ...
      <div
        class="card hidden"
        id="already-locked-card"
      >

        <p class="empty-msg">
          This match is already completed and locked.
          Delete and recreate it in Competitions to
          change the result.
        </p>
  ...
  function competitionName(id) {
    ...
    const c =
      allCompetitions.find(c => c.id === id);
  ...
  function scheduledDateTime(m) {
    ...
    return new Date(
      `${m.match_date}T${m.match_time || '00:00'}`
    );
  }
  ...
  function renderMatchListItem(m) {
    ...
    if (m.status === 'completed') {
      ...
      rightSideHtml = `
        <span class="status-pill score-pill">
          ${homeScore} – ${awayScore}
        </span>
      `;
      ...
      rightSideHtml = `
        <span class="status-pill status-postponed">
          Postponed
        </span>
      `;
      ...
      if (isLive) {
        ...
        actions.push(`
          <a
            ...
            href="${dashPath('/live-match')}?match=${m.id}"
          >
            🔴 Manage Live
          </a>
        `);
  ...
  async function showForm(id) {
    ...
    if (!hasKickedOff(currentMatch)) {
      ...
      document.getElementById(
        ...
        "This match hasn't kicked off yet";
      ...
      document.getElementById(
        ...
        }. Results can only be recorded once kickoff time has passed.`;
  ...
  function setupExternalForm() {
    ...
    const teamAName =
      teamName(currentMatch.team_a_id);

    const teamBName =
      teamName(currentMatch.team_b_id);
  ...
  function setupInternalForm() {
    ...
    document
      .getElementById('save-internal-btn')
      .addEventListener(
        'click',
        saveInternalResult
      );
  ...
  function renderSquadTable() {
    ...
    tbody
      .querySelectorAll('.squad-included')
      .forEach(cb => {
    ...
    tbody
      .querySelectorAll('.squad-position')
      .forEach(input => {
      ...
      subsList
        .map((sub, idx) => `
          <div class="dynamic-row">
            ...
            <button
              class="btn-danger remove-sub-btn"
              data-idx="${idx}"
            >
              Remove
            </button>
      ...
      cardsList
        .map((c, idx) => `
          <div class="dynamic-row">
            ...
            <button
              class="btn-danger remove-card-btn"
              data-idx="${idx}"
            >
              Remove
            </button>
      ...
      goalsList
        .map((g, idx) => `
          <div class="dynamic-row">
            ...
            <button
              class="btn-danger remove-goal-btn"
              data-idx="${idx}"
            >
              Remove
            </button>
  ...
  async function saveInternalResult() {
    ...
    try {
      ...
      statusEl.textContent =
        `Error: ${err.message}. ` +
        `Some data may have partially saved — ` +
        `check the match before retrying.`;
