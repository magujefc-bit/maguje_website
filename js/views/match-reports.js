...
export async function matchReportsView() {
  ...
  async function loadReports(root) {
    ...
    try {
      const { data, error } = await supabase.from('match_report_posts').select('id, slug, title, body, created_at').order('created_at', { ascending: false });
  ...
  async function renderGrid(root) {
    ...
    const cards = await Promise.all(visible.map(async p => {
      ...
      return newsCard({ slug: p.slug, title: p.title, excerpt: excerptFrom(p.body), coverImageUrl: cover, publishedAt: p.created_at }, { basePath: '/match-reports', badge: 'Match Report' });
    }));
