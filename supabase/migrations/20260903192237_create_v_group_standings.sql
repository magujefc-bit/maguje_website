...
WITH resolved AS (
  SELECT fr.competition_id,
         fr.group_name,
         CASE
             WHEN fr.team_key = 'own'::text THEN '00000000-0000-0000-0000-000000000000'::uuid
  ...
  SELECT r.competition_id,
     ...
     r.status
    FROM resolved r
   WHERE r.status = 'completed'::text AND r.team_id IS NOT NULL
), agg AS (
  SELECT c.competition_id,
     ...
     count(*) FILTER (WHERE c.goals_for > c.goals_against) * 3 + count(*) FILTER (WHERE c.goals_for = c.goals_against) AS points
    FROM completed c
   GROUP BY c.competition_id, c.group_name, c.team_id
)
...
ORDER BY a.competition_id, a.group_name, (rank() OVER (PARTITION BY a.competition_id, a.group_name ORDER BY a.points DESC, a.goal_difference DESC, a.goals_for DESC));
