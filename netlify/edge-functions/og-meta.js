...
// Normal visitors are NOT rewritten; they get the normal SPA response.
//
// Supported detail routes:
//   /news/:slug, /match-reports/:slug, /community/:slug, /events/:slug,
//   /gallery/:slug, /matches/:slug, /players/:slug, /competitions/:slug,
//   /officials/:slug
//
// Each route must also be attached in netlify.toml:
//   [[edge_functions]]
//     path = "/news/*"
//     function = "og-meta"
...
export default async (request, context) => {
  ...
  // Normal browser: don't touch the SPA response.
  ...
  // Not one of our supported detail routes.
  if (!route) {
    return context.next();
  }
  ...
  try {
    ...
    console.error("[og-meta] Metadata fetch failed:", error);
    // If Supabase fails, fall back to the normal SPA response.
    return context.next();
  }
  ...
  const description = cleanDescription(meta.description || DEFAULT_DESCRIPTION);

  // Always provide a valid image.
  const image =
    absoluteUrl(url, meta.image || DEFAULT_IMAGE) || DEFAULT_IMAGE;

  const pageUrl = canonicalPageUrl(url);
  ...
  // Response varies according to the User-Agent.
...
function matchRoute(pathname) {
  const table = [
    ...
    // NEW
    { re: /^\/officials\/([^/]+)\/?$/, build: buildOfficialMeta },
...
function absoluteUrl(pageUrl, path) {
  ...
  try {
    return new URL(value, pageUrl.origin).toString();
  } catch {
    return null;
  }
}
...
async function buildNewsMeta(slug) {
  ...
  return {
    ...
    description:
      excerptFrom(post.body) || `Read the latest news from ${SITE_NAME}.`,
    image,
    type: "article",
    post,
  };
...
async function buildMatchReportMeta(slug) {
  const rows = await sb("match_report_posts", {
    select: "id,title,body",
    slug: `eq.${slug}`,
    limit: "1",
  });
...
async function buildActivityMeta(slug) {
  const rows = await sb("activity_posts", {
    select: "id,title,body",
    slug: `eq.${slug}`,
    limit: "1",
  });
...
async function buildEventMeta(slug) {
  const rows = await sb("event_posts", {
    select: "id,title,body,location,event_date",
    slug: `eq.${slug}`,
    limit: "1",
  });
  ...
  if (post.event_date) {
    descriptionParts.push(String(post.event_date));
  }

  if (post.location) {
    descriptionParts.push(String(post.location));
  }
  ...
  return {
    title: `${post.title} | ${SITE_NAME}`,
    description: cleanDescription(`${prefix}${excerptFrom(post.body, 180)}`),
    image,
    type: "website",
    event: post,
  };
...
async function buildGalleryMeta(slug) {
  const rows = await sb("media_library", {
    select: "url,slug",
    slug: `eq.${slug}`,
    limit: "1",
  });
  ...
  return {
    ...
    description: `See photos from ${SITE_NAME} in our official gallery.`,
    image: item.url,
    type: "website",
    gallery: item,
  };
  ...
  return {
    ...
    description: isCompleted
      ...
      : `Upcoming fixture: ${homeLabel} vs ${awayLabel}${
          match.match_date ? ` on ${match.match_date}` : ""
        }.`,

    // Prefer the opponent logo if it exists; otherwise the default is used.
...
async function buildPlayerMeta(slug) {
  const rows = await sb("players", {
    select: "full_name,position,jersey_number,photo_url",
    slug: `eq.${slug}`,
    limit: "1",
  });
  ...
  if (player.position) {
    detailParts.push(player.position);
  }

  if (player.jersey_number !== null && player.jersey_number !== undefined) {
    detailParts.push(`#${player.jersey_number}`);
  }
...
// NEW
async function buildOfficialMeta(slug) {
  const rows = await sb("officials", {
    select: "full_name,official_role,photo_url,bio",
    slug: `eq.${slug}`,
    limit: "1",
  });
  ...
  return {
    title: `${official.full_name} | ${SITE_NAME}`,

    description: official.official_role
      ? `${official.full_name} — ${official.official_role} at ${SITE_NAME}.`
      : `${official.full_name} — club official at ${SITE_NAME}.`,

    image: official.photo_url,
...
async function buildCompetitionMeta(slug) {
  const rows = await sb("competitions", {
    select: "name,season",
    slug: `eq.${slug}`,
    limit: "1",
  });
  ...
  const seasonText = competition.season ? ` (${competition.season})` : "";

  return {
    title: `${competition.name}${seasonText} | ${SITE_NAME}`,
    description: `Fixtures, results and competition information for ${competition.name}${seasonText} — ${SITE_NAME}.`,
    image: null,
    type: "website",
    competition,
  };
...
function buildJsonLd({ route, title, description, image, pageUrl, meta }) {
  const teamId = `${SITE_URL}/#team`;
  ...
  if (
    ...
    const articleTitle = title.replace(` | ${SITE_NAME}`, "");
  ...
  // EVENT
  if (route.build === buildEventMeta && meta?.event) {
    const event = meta.event;

    graph.push({
      "@type": "Event",
      "@id": `${pageUrl}#event`,
      name: event.title || title.replace(` | ${SITE_NAME}`, ""),
      description: description,
      url: pageUrl,
      image: image ? [image] : undefined,
      organizer: { "@id": teamId },
  ...
  // GALLERY
  if (route.build === buildGalleryMeta) {
    graph.push({
      "@type": "ImageGallery",
      "@id": `${pageUrl}#gallery`,
      name: title,
  ...
  // PLAYER
  if (route.build === buildPlayerMeta && meta?.player) {
    ...
    graph.push({
      ...
      image: player.photo_url
        ? [absoluteUrl(new URL(pageUrl), player.photo_url)]
        : undefined,
      memberOf: { "@id": teamId },
  ...
  // OFFICIAL (NEW — fixed: uses new URL(pageUrl) instead of undefined `page`)
  if (route.build === buildOfficialMeta && meta?.official) {
    graph.push({
      "@type": "Person",
      "@id": `${pageUrl}#person`,
      name: meta.official.full_name,
      url: pageUrl,
      image: meta.official.photo_url
        ? [absoluteUrl(new URL(pageUrl), meta.official.photo_url)]
        : undefined,
      memberOf: { "@id": teamId },
      jobTitle: meta.official.official_role || undefined,
    });
  }

  // MATCH
  if (route.build === buildMatchMeta && meta?.match) {
    const opponentName = meta.opponent?.name || "TBD";

    const isHome = meta.match.is_home !== false;

    const homeTeam = isHome ? SITE_NAME : opponentName;
    const awayTeam = isHome ? opponentName : SITE_NAME;
    ...
    const matchEvent = {
      ...
      sport: "Football",
      organizer: { "@id": teamId },
      startDate: meta.match.match_date || undefined,
    };

    if (meta.match.status === "completed") {
      matchEvent.eventStatus = "https://schema.org/EventScheduled";
      matchEvent.description = description;
    }

    graph.push(matchEvent);
  }

  // COMPETITION
  if (route.build === buildCompetitionMeta && meta?.competition) {
    graph.push({
      "@type": "SportsEvent",
      "@id": `${pageUrl}#competition`,
      name: meta.competition.name,
      url: pageUrl,
      description: description,
      sport: "Football",
      organizer: { "@id": teamId },
...
function injectMeta(
  ...
  const safeType = escapeAttr(type || "website");
...
function injectJsonLd(html, data) {
  ...
  // Escape characters that could terminate or interfere with a script block.
  const json = JSON.stringify(data)
    .replaceAll("<", "\\u003c")
    .replaceAll(">", "\\u003e")
    .replaceAll("&", "\\u0026");

  const script = `<script type="application/ld+json" data-maguje-jsonld="true">${json}</script>`;
